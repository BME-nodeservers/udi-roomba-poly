...
class icons():
    ...
    def load_icon_file(self, name, filename, size=None):
        try:
            ...
            LOGGER.warning('Error loading icon file: {} : {}'.format(filename, e))
...
class Roomba(object):
    ...
    def __init__(self, address=None, blid=None, password=None, topic="#",
        ...
        self.icons = icons(base_icon=None, angle=self.angle, fnt=self.fnt, size=(32,32), log=LOGGER)
    ...
    def configure_roomba(self):
        ...
        LOGGER.warning('No Roomba specified, or found, exiting')
        return False
    ...
    async def async_connect(self):
        ...
        while not self.roomba_connected:
            try:
                ...
                if e.errno == 111:      #errno.ECONNREFUSED
                    LOGGER.error('Unable to Connect to roomba {}, make sure nothing else is connected (app?), '
                                   'as only one connection at a time is allowed'.format(self.roombaName))
                elif e.errno == 113:    #errno.No Route to Host
                    LOGGER.error('Unable to contact roomba {} on ip {}'.format(self.roombaName, self.address))
                else:
                    LOGGER.error("Connection Error: {} ".format(e))
                
                await asyncio.sleep(retry_timeout)
                LOGGER.error("Attempting retry Connection# {}".format(count))
        ...
        if not self.roomba_connected:   
            LOGGER.error("Unable to connect to {}".format(self.roombaName))
    ...
    async def _disconnect(self):
        ...
        LOGGER.info('{} disconnected'.format(self.roombaName))
    ...
    async def process_q(self):
        ...
        while True:
            try:
                if self.q.qsize() > 0:
                    LOGGER.warning('Pending event queue size is: {}'.format(self.q.qsize()))
                msg = await self.q.get()
                
                if not self.command_q.empty():
                    LOGGER.info('Command waiting in queue')
    ...
    async def periodic_update(self):
        ...
        while True:
            ...
            if self.roomba_connected:
                LOGGER.info("Publishing master_state")
    ...
    def on_disconnect(self, mosq, obj, rc):
        ...
        self.connected(False)
        if rc != 0:
            LOGGER.warning("Unexpected Disconnect! - reconnecting")
        else:
    ...
    def broker_on_connect(self, client, userdata, flags, rc):
        LOGGER.debug("Broker Connected with result code " + str(rc))
    ...
    def broker_on_message(self, mosq, obj, msg):
        ...
        if "command" in msg.topic:
            LOGGER.info("Received COMMAND: {}".format(payload))
            ...
            LOGGER.warn("Unknown topic: {}".format(str(msg.topic)))
            
    def set_simulate(self, value=False):
        if self.simulation != value:
            LOGGER.info('Set simulation to: {}'.format(value))
    ...
    def broker_on_disconnect(self, mosq, obj, rc):
        LOGGER.debug("Broker disconnected")
    ...
    def _send_command(self, command):
        ...
        LOGGER.info("Sending Command: {}".format(myCommand))
    ...
    def send_region_command(self, command):
        ...
        LOGGER.info('pmaps: {}'.format(pmaps))
        myCommand = {}
    ...
    def _set_preference(self, preference, setting):
        LOGGER.info("Received SETTING: {}, {}".format(preference, setting))
    ...
    def _set_cleanSchedule(self, setting):
        LOGGER.info("Received Roomba {} cleanSchedule:".format(self.roombaName))
        sched = "cleanSchedule"
        if self.is_setting("cleanSchedule2"):
            sched = "cleanSchedule2"
        Command = {"state": {sched: setting}}
        myCommand = json.dumps(Command)
        LOGGER.info("Publishing Roomba {} {} : {}".format(self.roombaName, sched, myCommand))
        self.client.publish("delta", myCommand)
    
    def publish(self, topic, message):
        if self.mqttc is not None and message is not None:
            topic = '{}/{}'.format(self.brokerFeedback, topic)
            LOGGER.debug("Publishing item: {}: {}".format(topic, message))
    ...
    def set_options(self, raw=False, indent=0, pretty_print=False, max_sqft=0):
        ...
        if self.raw:
            LOGGER.info("Posting RAW data")
        else:
            LOGGER.info("Posting DECODED data")
    ...
    def _enable_map(self, enable=False, mapSize="(800,1500,0,0,0,0)",
        ...
        if enable:
            LOGGER.info("MAP: Maps Enabled")
            if isinstance(mapSize, str):
                mapSize = literal_eval(mapSize)
            self.mapSize = mapSize
            if len(self.mapSize) < 6:
                LOGGER.error("mapSize is required, and is of the form "
            ...
            if self.fnt is None:
                try:
                    ...
                    LOGGER.warning("error loading font: %s, loading default font".format(e))
            ...
            if not roomOutline:
                LOGGER.info("MAP: Not drawing Room Outline")
            ...
            if floorplan:
                ...
                if floorplan and len(floorplan) != 6:
                    LOGGER.error('floorplan is of the form '
    ...
    def load_floorplan(self, filename, new_center=(0,0), scale=None, angle=0, transparency=0.2):
        if self.base is not None:
            LOGGER.info('MAP: loading floorplan: {}'.format(filename))
            try:
                ...
                LOGGER.info('loaded floorplan: {}'.format(filename))
            except Exception as e:
                LOGGER.warning("MAP: unable to load {}: {}".format(filename,e))     
    ...
    def update_state_machine(self, new_state = None):
        ...
        if new_state is not None:
            ...
            LOGGER.info("set current state to: {}".format(self.current_state))
            self.draw_map(True)
            return    
        ...
        LOGGER.info('current_state: {}, current phase: {}, mission: {}, mission_min: {}, recharge_min: {}, co-ords changed: {}'.format(self.current_state,
                                                                                                                    phase,
                                                                                                                    mission,
                                                                                                                    self.mssnM,
                                                                                                                    self.rechrgM,
        ...
        if self.current_state == self.states["new"] and phase != 'run':
            ...
            try:
                ...
                LOGGER.warning('phase: {} not found in self.states'.format(phase))

        if self.current_state != current_mission:
            LOGGER.info("updated state to: {}".format(self.current_state))

        self.publish("state", self.current_state)
        
        if self.is_set('ignore_coordinates') and self.current_state != self.states["new"]:
            LOGGER.info('Ignoring co-ordinate updates')
        else:
    ...
    def clear_outline(self):
        ...
        LOGGER.info('Erased room outline image')
            
    def load_image(self, name, make_none=False):
        LOGGER.info("MAP: opening existing {}".format(name))
        type = name.split('.')[-1]
        ...
        if type == 'npy':
            try:
                ...
                LOGGER.warning("Unable to load {}: {}, setting "
            ...
            try:
                ...
                if make_none:
                    LOGGER.warning("MAP: unable to load {}: {}: set to None".format(name, e))
                    return None
                else:
                    LOGGER.warning("MAP: unable to load {}: {}: "
    ...
    def save_image(self, var, name='', final_name=None):
        if var is None or '.' not in name:
            LOGGER.warning('invalid save_image attempt')
            return
        LOGGER.debug("MAP: saving {}".format(final_name if final_name else name))
        type = name.split('.')[-1]
        filename = '{}/{}{}'.format(self.mapPath, self.roombaName, name)
        if type == 'npy':
    ...
    def initialise_map(self, roomba_size):
        ...
        LOGGER.info("MAP: home_pos: ({},{})".format(self.home_pos[0], self.home_pos[1]))
            
        self.dock_position = (
            self.home_pos[0] - self.icons['home'].size[0] // 2,
            self.home_pos[1] - self.icons['home'].size[1] // 2)

        LOGGER.info("MAP: Initialisation complete")
    ...
    def draw_vacuum_lines(self, image, old_x_y, x_y, theta):
        ...
        if old_x_y is None or x_y is None or \
           ...
           old_x_y[0] < 0 or old_x_y[1] < 0:
            LOGGER.warning('MAP: Not drawing line {}, {}: value out of range'.format(old_x_y, x_y))
            return
        if self.distance_between(x_y, old_x_y) > self.max_distance:
            LOGGER.warning('MAP: Not drawing line {}, {}: distance is greater than {}'.format(old_x_y, x_y, self.max_distance))
            return
        lines = ImageDraw.Draw(image)
        if x_y != old_x_y:
            LOGGER.info("MAP: drawing line: {}, {}".format(old_x_y, x_y))
    ...
    def draw_text(self, image, display_text, fnt, pos=(0,0),
        ...
        LOGGER.info("MAP: writing text: pos: {}, max_len: {}, text: {}".format(pos, max_len, display_text))
        if rotate:
    ...
    def draw_roomba(self, roomba_pos, theta):
        ...
        LOGGER.info("MAP: drawing roomba: pos: {}, theta: {}".format(roomba_pos, theta))
    ...
    def render_map(self):
        ...
        if self.show_final_map == False:
            LOGGER.info("MAP: received: new co-ords: {} "
        ...
        if  self.current_state == self.states["charge"]:
            ...
            if self.is_set('update_after_completed'):
                LOGGER.info('not updating map/text (mission complete), resume in {}s'.format(self.when_run('update_after_completed')))
            else:
            ...
            if self.co_ords == self.zero_coords(theta=0):
                ...
                LOGGER.warning('MAP: received 0,0,0 pose when running - ignoring')
                self.old_x_y = None
                return
            ...
            LOGGER.info("MAP: created new image at start of new run")
            self.clear_flags()
            ...
            LOGGER.warning("MAP: no special handling for state: {}".format(self.current_state))

        if self.base is None:
            LOGGER.warning("MAP: no image, exiting...")
            return
        ...
        if self.show_final_map and not self.debug: #just display final map - not live
            LOGGER.debug("MAP: not updating map - Roomba not running")
            return
        ...
        if x_y is None:
            ...
            LOGGER.info("MAP: ignoring new co-ords in {} phase: {}".format(self.current_state, self.co_ords))
        ...
        LOGGER.debug("MAP: old x,y: {} new x,y: {} theta: {} roomba pos: {}".format(old_x_y, x_y, theta, roomba_pos))
    ...
    def draw_room_outline(self, overwrite=False, x_y=(0,0)):
        ...
        if not self.roomOutline:
            return
        LOGGER.debug("MAP: checking room outline")
        if HAVE_CV2:
            ...
            if cv2.pointPolygonTest(self.room_outline_contour, x_y, False) == -1:
                LOGGER.info("MAP: found new outline perimeter")
        ...
        if overwrite or self.debug:
            ...
            LOGGER.info("MAP: Wrote new room outline files")
    ...
    def transform_image(self, image):
        LOGGER.info("MAP: calculation of center: ({},{}), "
    ...
    def PIL_get_image_parameters(self, image=None, start=90, end = 0, step=-1,
        ...
        # LOGGER.info("MAP: PIL: image center: "
    ...
    def get_image_parameters(self, image=None, contour=None, final=False):
        ...
        if final:
            ...
            self.cy = x_y[1]
            self.angle = angle
        LOGGER.info("MAP: image center: x:{}, y:{}, angle {:.2f}".format(
                      x_y[0], x_y[1], angle))
    ...
    def draw_final_map(self, overwrite=False):
        ...
        if HAVE_CV2:
            ...
            try:
                ...
                LOGGER.warning("MAP: unable to remove contour")
                pass
        ...
        if overwrite:
            LOGGER.info("MAP: Drawing final map")
